...
//
use BlockDistRes, Time, Types, Random;
...
*/	
  ...
  for trial in 1..numTrials {                        // loop over the trials
    ...
    //
    writeln("--- main zippered loop----\n");

    forall (a, b, c) in zip(A, B, C) do {   
      a = b + alpha * c;
      write( here.id, "::  a=", a, " b=", b, " c=", c);
	}
  ...
  //writeln("Validation: ", if validAnswer then "SUCCESS" else "FAILURE");
...
proc initVectors(B, C) {
  ...
  /* module Random / class RandomStream
  ...
  * 
  * 
  *  proc fillRandom2(arr: [] eltType) {
    for (x, r) in zip(arr, iterate(arr.domain, arr.eltType)) do
      x = r;
  }
  * */
